									...
									<div class="table-row-group">
										<ul class="table-row" v-for="item in deviceObj" :key="item.deviceId">
											<li class="table-cell device-num">
												<img src="@/assets/images/monitorManage/monitor-3.png" class="monitor-device-img-1" v-if="item.type=='N'" />
												<img src="@/assets/images/monitorManage/monitor-4.png" class="monitor-device-img-2" v-else />
												<span class="monitor-device-text">{{item.deviceId}}</span>
											</li>
											<li class="table-cell">
												<span class="monitor-device-symbol" :class="[item.cameraRunStatus==1?'online':'offline']"></span>
											</li>
											<li class="table-cell">
												<div class="c-switch-style" :class="[item.value?active:unActive]" @click="switchChange(item)" v-show="item.type=='N'">
													<i></i>
												</div>
											</li>
										</ul>
		...
		data() {
			return {
				...
				option: {
					...
					flash: {
						swf: isProduction ?
							"/monPlatform/static/media/video-js.swf" : "/static/media/video-js.swf"
					},
					sources: [{
						type: "rtmp/mp4",
						src: ""
		...
		methods: {
			...
			updateCameraPosition(){
				if(this.heading){
					...
					this.$refs.tusvnMap3.updateCameraPosition1(this.position[0], this.position[1], 25, 13, -0.2, -1.9+(Math.PI / 180.0)*this.heading);
				}else{
					this.$refs.tusvnMap3.updateCameraPosition1(this.position[0], this.position[1], 25, 13, -0.2, 0.97 + (Math.PI / 180.0) * 180);
				}
...
<style lang="scss" scoped>
	...
	.m-side-dialog-content {
		...
		.side-device-list {
			...
			height: auto;
			padding: 20px 0 20px 20px;
			background: linear-gradient( 90deg, rgba(0, 0, 0, 0), rgba(0, 0, 0, 0.5) 70%);
			/* 标准的语法 */
			.device-list-style {
				background-color: #000;
				.table-title{
